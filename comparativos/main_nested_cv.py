...
def evaluate_model(epochs, batch_size, learning_rate, 
    ...
    output = model.evaluate(Xvalid,Yvalid)
...
ifold = KFold(n_splits=(num_folds-1), shuffle=True)
...
if __name__ == '__main__':
    ...
    history = pd.DataFrame(columns=('ID', 'testfold', 'validationfold', 'exp_mean_loss', 'loss', 'accuracy', 'time'))
    df_results = pd.read_csv(exp_name+" geral.csv",sep=";",decimal=",")
    for index, linha in df_results.iterrows():
       #learning rate = 10 ** -li
       learnrate = 10**(-int(linha["learningIndex"]))
       #batch size = 2 ** bti
       batchsize = 2**(int(linha["batchIndex"]))
       #multiple layers params
      
       convLayers = int(linha["camadasConvolucionais"])
       maxPooling = []
       batchNorm = []
       numFilters = []
       sizeFilters = []
       ...
       denseLayers = int(linha["camadasDensas"])
       denseNeurons = []
       for d in range(denseLayers-1):
