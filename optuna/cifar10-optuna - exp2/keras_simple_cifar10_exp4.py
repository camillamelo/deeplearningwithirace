...
def objective(trial):
    ...
    max_pooling = trial.suggest_categorical("maxpooling0", [True, False])
    batchnormalization = trial.suggest_categorical("batchnormalization0", [True, False])
    n_pooling = 0
    if max_pooling:
    ...
    for i in range(n_conv_layers-1):
        filters_ind = trial.suggest_int("filtersind{}".format(i+1), 5, 8) #expoente do número de filtros
        model.add(
            Conv2D(
                filters=2 ** filters_ind,
                kernel_size=trial.suggest_categorical("kernel_size{}".format(i+1), [3, 5, 7, 9, 11]),
                activation="relu",
                padding='same'
            )
        )
        if n_pooling < 4: #maximo 4 max_poolings pois divide a entrada pela metade e 32 = 2 ^ 4
            max_pooling = trial.suggest_categorical("maxpooling{}".format(i+1), [True, False])
            if max_pooling:
                model.add(MaxPooling2D())
                n_pooling = n_pooling + 1
        batchnormalization = trial.suggest_categorical("batchnormalization{}".format(i+1), [True, False])
        if batchnormalization:
            model.add(BatchNormalization())
...
if __name__ == "__main__":
        ...
        try:
            ...
            for gpu in gpus:
                ...
                tf.config.set_logical_device_configuration(gpu,
                                                           [tf.config.LogicalDeviceConfiguration(memory_limit=6656)])
    ...
    storage = JournalStorage(JournalFileStorage("optuna-journal4.log"))
    #get date and time
    #now = datetime.now()
    #date_time = now.strftime("%Y-%m-%d-%H-%M-%S")

    study_name = "Experiment4"
    ...
    study.optimize(objective, timeout=131556, n_jobs=1, show_progress_bar=True)
